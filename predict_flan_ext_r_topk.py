...
adapter_name = "/scratch/atr1n17/COLING2022/models/idn_corrected_tokens/flan_ext_r2/checkpoint-2500"
...
for i,entry in enumerate(data):
  ...
  with codecs.open(os.path.join(save_path, 'hyp.txt'), 'a', encoding='utf-8', errors='ignore') as f:
        f.write(summary + '<<END>>')
    #preds = [probs[i].float() for i in range(0, 25, 1) if len(probs)>i and probs[i]>0.3]
    #    output_text = [options[i] for i in range(0, 25, 1) if len(options)>i and probs[i] > 0.3 ]
    #    all_probs.append(preds)
    #    summary.append(output_text)

  #k_largest = heapq.nlargest(81, all_probs)
  #indices = [all_probs.index(x) for x in k_largest]
  #indices.sort()
  #summary_sentences = [sentence for sentence in summary if summary.index(sentence) in indices]
  #text_summary = ' '.join(summary_sentences)

  #with codecs.open(os.path.join(save_path, 'hyp.txt'),'a', encoding='utf-8', errors='ignore') as f:
  #    f.write(text_summary+'<<END>>')
